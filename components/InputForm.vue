<template>
    <Field v-model="modelValue" :name="name" :type="type" :rules="rules" validate-on-input
        v-slot="{ field, errorMessage }">
        <div>
            <p class="font-bold my-2 px-1 md:text-base text-sm">{{ label }}</p>
            <div class="relative flex items-center justify-center">
                <input :type="type" v-bind="field"
                    class="w-full pl-3 pr-3 py-3 border-2 text-sm rounded-full focus:outline-none focus:ring-1 focus:ring-primary-1"
            ...
            <transition name="fade">
                <span v-if="errorMessage" class="error-message font-bold text-red-2 md:text-sm text-xs">{{
                    errorMessage }}</span>
            </transition>
        </div>
    </Field>
