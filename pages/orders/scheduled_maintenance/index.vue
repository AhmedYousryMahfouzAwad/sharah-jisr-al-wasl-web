<template>
    <div class="xl:mx-auto mt-5 ">
        ...
        <OrdersTabs :tabs="[
            { value: 3 || 4, label: t('pages.new') },
            { value: 5 || 6, label: t('pages.current') },
            { value: 9, label: t('pages.Finished') }
        ]" v-model="activeTab" @update:model-value="setActiveTab(1, activeTab)" />
        ...
        <div class="tab-content mt-4 ">
            <div class="tab-content mt-4 max-w-5xl mx-auto pb-10">
                <div
                    class="grid gap-x-6 grid-cols-12 mx-auto sm:grid-cols-6 md:grid-cols-6 xl:grid-cols-6 lg:grid-cols-6">
                    ...
                    <OrdersCard v-if="!loading && list_orders.length > 0" v-for="item in list_orders" :key="item.id"
                        ...
                        :to="localeRoute({ name: 'orders-scheduled_maintenance-id', params: { id: item.id } })" />
...
const { list_orders, pagination } = storeToRefs(useOrderPackageStore());
const { getOrder } = useOrderPackageStore();
